...
TARGET_CFG_FILE = projects/$(T)/main/target.mk
TARGET_COMMON_FILE = projects/common.mk
...
KBUILD_OUTPUT_X := $(shell mkdir -p $(KBUILD_OUTPUT) && cd $(KBUILD_OUTPUT) \
                         && /bin/pwd)
endif
...
ARCH            ?= arm
CROSS_COMPILE   ?= $(KBUILD_ROOT)/toolchain/gcc-arm-none-eabi-9-2020-q2-update-win32/bin/$(CONFIG_CROSS_COMPILE:"%"=%)
...
KBUILD_CFLAGS   := \
    -fno-common -fmessage-length=0 -Wall \
...
ifeq ($(LDS_FILE),)
$(error LDS_FILE should be defined in target.mk)
endif
...
STR_BIN   := $(addsuffix .str.bin,$(basename $(IMAGE_FILE)))
...
ifneq ($(T), boot)
	@$(PWD)/tools/xMerge/merge_file.exe -s ../boot/boot.bin -t $(KBUILD_OUTPUT)/$(T).bin -p $(VECT_TAB_OFFSET) -o $(KBUILD_OUTPUT)/all.bin
endif
...
all: $(IMAGE_BIN) $(STR_BIN) IMAGE_DFU;
else
all: $(IMAGE_BIN) ;
endif
...
#    cmd_gen-STR_BIN = $(OBJCOPY) \
#          -j .code_start_addr --change-section-lma .code_start_addr=0x00008000 \
#          -j .rodata_str --change-section-lma .rodata_str=0x00000010 \
#          -j .trc_str --change-section-lma .trc_str=0x00000000 \
#          -O binary $< $@
     cmd_gen-STR_BIN = $(OBJCOPY) \
           -j .trc_str --change-section-lma .trc_str=0x00000000 \
           -O binary $< $@
...
#$(IMAGE_VER): $(#) $(filter-out $(IMAGE_VER),$(IMAGE-deps)) FORCE
...
ifeq ($(LDS_SRC_STEM),)
$(error LDS_SRC_STEM may be defined in target.mk)
endif

LDS_SRC := $(firstword $(wildcard $(LDS_SRC_STEM).S $(LDS_SRC_STEM).sx))
...
cmd_link-IMAGE_FILE = $(LD) -o $@ \
    ...
    -Wl,--no-whole-archive $(LIB_LDFLAGS)
endif
